...
let syntaxRuleError = (token) => {
  return new SyntaxError(
    `Found: ${token.value} token, expected one of: !=, &&, (, *, **, +, -, ., /, <, <=, ==, >, >=, [, in, ||`
  );
};
...
function checkUndefinedProperties(template, allowed) {
  ...
  if (unknownKeys) {
    throw new TemplateError(
      allowed[0].replace("\\", "") + " has undefined properties:" + unknownKeys
    );
  }
}
  ...
  while ((offset = remaining.search(/\$?\${/g)) !== -1) {
    ...
    if (remaining[offset + 1] != "$") {
      ...
      if (isArray(v.result) || isObject(v.result)) {
        ...
        throw new TemplateError(
          `interpolation of '${input}' produced an array or object`
        );
      }
...
operators.$if = (template, context) => {
  ...
  if (isTruthy(parse(template["$if"], context))) {
    if (template.hasOwnProperty("$then")) {
      throw new TemplateError(
        "$if Syntax error: $then: should be spelled then: (no $)"
      );
    }
  ...
  return template.hasOwnProperty("else")
    ? render(template.else, context)
    : deleteMarker;
};
...
operators.$map = (template, context) => {
  ...
  let match =
    /^each\(([a-zA-Z_][a-zA-Z0-9_]*)(,\s*([a-zA-Z_][a-zA-Z0-9_]*))?\)$/.exec(
      eachKey
    );
  if (!match) {
  ...
  if (object) {
    ...
    value = value
      .map((v) => {
        let args =
          typeof i !== "undefined" ? { [x]: v.val, [i]: v.key } : { [x]: v };
        eachValue = render(each, Object.assign({}, context, args));
        if (!isObject(eachValue)) {
          throw new TemplateError(
            `$map on objects expects each(${x}) to evaluate to an object`
          );
        }
        return eachValue;
      })
...
operators.$reduce = (template, context) => {
  const EACH_RE =
    "each\\(([a-zA-Z_][a-zA-Z0-9_]*),\\s*([a-zA-Z_][a-zA-Z0-9_]*)(,\\s*([a-zA-Z_][a-zA-Z0-9_]*))?\\)";
  ...
  let eachKey = Object.keys(template).find(
    (k) => k !== "$reduce" && k !== "initial"
  );
  let match =
    /^each\(([a-zA-Z_][a-zA-Z0-9_]*),\s*([a-zA-Z_][a-zA-Z0-9_]*)(,\s*([a-zA-Z_][a-zA-Z0-9_]*))?\)$/.exec(
      eachKey
    );
  if (!match) {
...
operators.$mergeDeep = (template, context) => {
  ...
  let merge = (l, r) => {
    ...
    if (isObject(l) && isObject(r)) {
      ...
      for (let p in r) {
        // eslint-disable-line taskcluster/no-for-in
        if (p in l) {
          res[p] = merge(l[p], r[p]);
        } else {
          res[p] = r[p];
...
operators.$reverse = (template, context) => {
  ...
  if (!isArray(value)) {
    throw new TemplateError(
      "$reverse value must evaluate to an array of objects"
    );
  }
...
let render = (template, context) => {
  ...
  let matches = Object.keys(operators).filter((c) =>
    template.hasOwnProperty(c)
  );
  if (matches.length > 1) {
  ...
  for (let key of Object.keys(template)) {
    ...
    if (value !== deleteMarker) {
      ...
      if (key.startsWith("$$")) {
        processedKey = key.substr(1);
      } else if (/^\$[a-zA-Z][a-zA-Z0-9]*$/.test(key)) {
        throw new TemplateError(
          "$<identifier> is reserved; use $$<identifier>"
        );
      } else {
        processedKey = interpolate(key, context);
      }

      // Process conditional keys
      let isConditionalKey = processedKey.startsWith("?");
      let finalKey = isConditionalKey ? processedKey.substr(1) : processedKey;
...
module.exports = (template, context = {}) => {
  ...
  let test = Object.keys(context).every((v) =>
  ...
  context = addBuiltins(
    Object.assign({}, { now: fromNow("0 seconds") }, context)
  );
